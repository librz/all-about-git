gdb () {
	...
	branches_to_delete=$(echo "$branches_raw" | awk -F'|' 'NF {print $1}' | tr '\n' ' ')
	if [[ "$repo_type" -eq 1 ]]
	then
		cmd="git branch -D $branches_to_delete"
	else
		cmd="git push "$remote" --no-verify --delete $branches_to_delete"
	fi
	echo
